...
import { getAnalyticsOverviewAction } from "@/features/analytics/server/query-actions"
import { analyticsQueryKeys } from "@/lib/query-keys"

type AnalyticsPageQueryProps = {
  initialData: Awaited<ReturnType<typeof getAnalyticsOverviewAction>>
}

export function AnalyticsPageQuery({
  initialData,
}: Readonly<AnalyticsPageQueryProps>) {
  const queryClient = useQueryClient()
  const queryKey = React.useMemo(() => analyticsQueryKeys.overview(), [])
  ...
  const analytics = query.data ?? initialData
    ...
    <div className="flex flex-col gap-4">
      {query.isError ? (
        <div className="rounded-2xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-sm text-destructive">
          Could not refresh analytics right now. Showing the latest loaded snapshot.
        </div>
      ) : null}
